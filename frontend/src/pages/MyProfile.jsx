    ...
    return userData ? (
        <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
            ...
            <div className='mt-8 w-full'>
                <p className='text-[#797979] underline mt-3'>Medical Adherence</p>
                <div className='mt-3 flex flex-col gap-3'>
                    {userData.medicalAdherence && userData.medicalAdherence.length > 0 ? (
                        userData.medicalAdherence.map((entry, idx) => (
                            <div key={idx} className='p-3 border rounded'>
                                <p className='text-sm text-gray-600'>Date: {new Date(entry.date).toLocaleString()}</p>
                                {entry.medicines && entry.medicines.map((m, i) => (
                                    <div key={i} className='mt-2'>
                                        <p className='font-medium'>{m.name} — {m.dosage}</p>
                                        <p className='text-sm text-gray-600'>Times: {m.times ? m.times.join(', ') : ''} • {m.meal} meal • {m.days} days</p>
                                    </div>
                                ))}
                            </div>
                        ))
